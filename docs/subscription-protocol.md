...
| `weight` | — | 同组排序，数字小者靠前；缺省按标题 |
| 正文 | — | 策展理由（为什么订、要看什么），随源保存为备注 |
...
内容知识库插件在一次 sync 之后，把**所有有效源**的订阅合并写成一个文件：
...
| 本地已有该 URL，且是知识库托管 | **更新**分组 / 标签 / 备注 / `enabled` / `title`（`title` 仅在源标题仍来自 feed 或 url 时覆盖） |
| 本地已有该 URL，但是**手动加的** | **不接管**：只更新分组为空时的建议，其余字段保持用户意图；标记为"双份"并在同步结果里报告 |
| 清单里已消失（上次导入过、这次没有了） | 标记 `stale: true`，**保留已抓文章**，列表里显示"知识库已移除"，由用户决定是否删除 |
...
- **停用**（`enabled: false`，或桌面手动停用）：源还在，文章还在，只是不再抓取；
- **知识库移除**（文件被删）：标 `stale`，文章保留，提示用户；
...
| 阶段 | 内容 | 产出 |
|---|---|---|
| P0（本次） | 仓库标准 + `feeds/` 协议 + 真实仓库（含 4 条真实订阅） | 本仓库 |
| P1 | 知识库插件：扫 `feeds/`、写 `knowledge/feeds.json`、设置页计数 | `feeds.json` 真的出现 |
| P2 | 灵感插件：导入 + 徽标 + 「从知识库同步」+ 刷新前静默导入 | 订阅自动出现并抓取 |
| P3 | 导出草稿 + 自动化接线（每日 `inspiration__refresh`，知识库 sync 后触发导入） | 闭环 |
