...
- 这两件事你都能在本机覆盖：右键可**停用/启用**（你的开关优先于仓库的 `enabled`，
  且不被同步抹掉），也可以**直接删除**一个仍被声明的源——本机会记住这次忽略，
  下次同步不会把它订阅回来，随时可用轨道上的「恢复被忽略的订阅」撤销；
- **整个知识库删掉** → 已订阅的源**原样保留**，从此是你自己的源，随时可删。
- 只写**公网地址**：环回 / 内网 / 链路本地（`localhost`、`127.x`、`10.x`、`192.168.x`、
  `169.254.x`、`::1`、`.local`…）会被跳过并在设置页报原因——清单是共享的，
  桌面会照着它发请求；内网源请在面板里手动添加。
